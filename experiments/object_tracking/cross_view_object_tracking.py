...
def cross_view_calculate_pr_curves(object_descs_list_view1, object_descs_list_view2, intervals):
  pr_curves = {}
  for interval in intervals:
    gts, matches = [], []
    last_object_descs = None
    for i in range(len(object_descs_list)):
      ...
      object_descs_view1 = object_descs_list_view1[i]
      object_descs_view2 = object_descs_list_view2[i]

      if last_object_descs_view2 is not None and object_descs_view1 is not None:
        gt_matrix, match_matrix = match_objects(object_descs_view1, last_object_descs_view2)
        if gt_matrix is not None and match_matrix is not None:
            gts += gt_matrix
            matches += match_matrix

      last_object_descs_view2 = object_descs_view2
...
def single_object_tracking(configs):
  ...
  for seq in SEQNAMES:
    ...
    object_descs_list_view1 = []
    object_descs_list_view2 = []
    ...
    for idx,data in enumerate(dataset):
      ...
      if idx<self.len_image1:
        ...
        object_descs_list_view1.append(None)
        continue

        object_descs = reorder_descs(net_output, tracking_gt)
        object_descs_list_view1.append(object_descs)
        ...
        tracking_gt = dataset.get_label(image_name)
        if tracking_gt is None or net_output['points'] is None:
        object_descs_list_view2.append(None)
        continue

        object_descs = reorder_descs(net_output, tracking_gt)
        object_descs_list_view2.append(object_descs)
    
    # pr_curves = calculate_pr_curves(object_descs_list, intervals)
    pr_curves = cross_view_calculate_pr_curves(object_descs_list_view1, object_descs_list_view2, intervals)
